          ...
          <div className='w-full flex items-center justify-start gap-6 my-5'>
            {
              filterProjects.map((item, key) => (
                <button key={key} className={`font-medium ${isFilterActive === item ? 'text-orange-yellow-crayola' : 'text-light-gray'} text-base md:text-lg transition-colors`} onClick={() => handleFilterProject(item)}>{item}</button>
              ))
            }
          </div>
        </div>
